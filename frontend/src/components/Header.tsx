...
const sectionOptions = [
  { view: 'workspace' as const, label: 'Lab Workspace', description: 'Build and run circuits', icon: BookOpen },
  { view: 'learning-path' as const, label: 'Learning Path', description: 'Follow your roadmap', icon: Compass },
  { view: 'theory-math' as const, label: 'Theory Lab', description: 'Study concepts and math', icon: BrainCircuit },
  { view: 'student-dashboard' as const, label: 'Student Portal', description: 'Review your progress', icon: UserCheck },
  { view: 'student-analysis' as const, label: 'Student Analysis', description: 'Understand your performance', icon: LineChart }
];
      ...
      <div className="h-13 px-4 flex items-center justify-between border-b border-slate-200">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="relative">
            <button
              className="flex items-center space-x-2.5 cursor-pointer group select-none text-left"
              onClick={() => setIsSectionMenuOpen((open) => !open)}
              aria-expanded={isSectionMenuOpen}
              aria-label="Open QLearn sections"
            >
            ...
            <div>
              <div className="flex items-center space-x-1.5">
                <span className="font-black text-sm tracking-tight text-slate-900 font-sans uppercase">
                  Q<span className="text-blue-600 font-bold">LEARN</span>
                </span>
                <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200 font-bold">
                  v3.0
                </span>
                <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isSectionMenuOpen ? 'rotate-180' : ''}`} />
              </div>
              <p className="text-[10px] text-slate-500 font-medium">Quantum learning platform</p>
            </div>
            </button>

            {isSectionMenuOpen && (
              <div className="absolute left-0 top-[calc(100%+10px)] z-50 w-64 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-1.5 shadow-xl">
                <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] uppercase tracking-wider font-bold text-blue-600 dark:text-cyan-400">QLearn sections</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Choose where to continue</p>
                </div>
                {sectionOptions.map(({ view, label, description, icon: Icon }) => (
                  <button
                    key={view}
                    onClick={() => {
                      setActiveView(view);
                    ...
                    className="w-full flex items-center gap-3 rounded-lg px-2.5 py-2 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                  >
                    <span className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-cyan-300 flex items-center justify-center">
                      <Icon className="w-3.5 h-3.5" />
                    </span>
                    <span className="min-w-0">
                      <span className="block text-xs font-bold text-slate-800 dark:text-slate-100">{label}</span>
                      <span className="block text-[10px] text-slate-500 dark:text-slate-400 truncate">{description}</span>
                    </span>
                  </button>
                ))}
              </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="w-8 h-8 rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white transition-all flex items-center justify-center shadow-sm"
