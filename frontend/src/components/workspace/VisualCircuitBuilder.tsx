...
import { Plus, Minus, Cpu, Code, Sliders, Maximize2, Minimize2, Trash2 } from 'lucide-react';
...
export const VisualCircuitBuilder: React.FC = () => {
  ...
  return (
    <div id="circuit-builder-panel" className="h-full flex flex-col bg-white overflow-hidden">
      {/* Gate Palette Bar & Dynamic Grid Controls */}
      <div className="p-2.5 bg-white border-b border-slate-200 flex items-center justify-between flex-wrap gap-2 shadow-xs z-10">
        <div className="flex items-center space-x-2 flex-wrap gap-1.5">
          ...
          <div className="flex items-center space-x-0.5 bg-slate-100 p-1 rounded-lg border border-slate-200">
            {/* Standard Pauli / Hadamard */}
            {(['H', 'X', 'Y', 'Z'] as GateType[]).map((g) => (
              <button
                key={g}
                onClick={() => {
            ...
            {/* Phase / Rotation */}
            {(['S', 'T', 'RX', 'RY', 'RZ'] as GateType[]).map((g) => (
              <button
                key={g}
                onClick={() => {
                  setSelectedGateType(g);
            ...
            {(['CNOT', 'CZ', 'TOFFOLI'] as GateType[]).map((g) => (
              <button
                key={g}
                onClick={() => {
                  setSelectedGateType(g);
        ...
        {/* Dynamic Grid (Rows & Columns), QASM, and Fullscreen Controls */}
        <div className="flex items-center space-x-2 flex-wrap gap-1.5">
          {/* Row (Qubits) Control */}
          <div className="flex items-center space-x-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-200 text-xs font-mono">
            <span className="text-slate-700 font-semibold">Rows (Qubits):</span>
            <span className="px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-bold text-[11px]">{qubitCount}</span>
            <button
              onClick={removeQubitRow}
              disabled={qubitCount <= 1}
              className="p-1 rounded hover:bg-slate-200 text-slate-700 disabled:opacity-30 cursor-pointer"
              title="Remove Row (Qubit wire)"
            >
              <Minus className="w-3 h-3" />
            </button>
            <button
              onClick={addQubitRow}
              disabled={qubitCount >= 8}
              className="p-1 rounded hover:bg-slate-200 text-slate-700 disabled:opacity-30 cursor-pointer"
              title="Add Row (Qubit wire)"
            >
              <Plus className="w-3 h-3" />
            </button>
          </div>

          {/* Column (Steps) Control */}
          <div className="flex items-center space-x-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-200 text-xs font-mono">
            <span className="text-slate-700 font-semibold">Cols (Steps):</span>
            <span className="px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 font-bold text-[11px]">{stepCount}</span>
            <button
              onClick={removeStepColumn}
              disabled={stepCount <= 4}
              className="p-1 rounded hover:bg-slate-200 text-slate-700 disabled:opacity-30 cursor-pointer"
              title="Remove Column (Step slice)"
            >
              <Minus className="w-3 h-3" />
            </button>
            <button
              onClick={addStepColumn}
              disabled={stepCount >= 24}
              className="p-1 rounded hover:bg-slate-200 text-slate-700 disabled:opacity-30 cursor-pointer"
              title="Add Column (Step slice)"
            >
              <Plus className="w-3 h-3" />
            </button>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-1 px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-xs text-indigo-700 hover:bg-indigo-50 shadow-xs cursor-pointer"
          >
            <Code className="w-3.5 h-3.5 text-indigo-600" />
            <span>QASM</span>
          </button>

          {/* Full Screen Mode Toggle Button (Only shown when not in fullscreen; exit is in the top header) */}
          {!isFullscreenWorkspace && (
            <button
              onClick={() => setIsFullscreenWorkspace(true)}
              className="flex items-center space-x-1.5 px-3 py-1 rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white"
              title="Enter Full Screen Studio with Live Quantum Preview"
            >
      ...
      {/* Stave Circuit Canvas Grid - Dynamic Scrollable Area */}
      <div className="flex-1 overflow-auto p-6 flex flex-col items-start quantum-grid-bg relative select-none">
          ...
          <div className="space-y-6">
            {Array.from({ length: qubitCount }, (_, qubitIdx) => (
              <div key={qubitIdx} className="flex items-center relative">
                <div className="w-20 font-mono text-xs font-semibold text-blue-600 flex items-center space-x-2 flex-shrink-0">
                  <span className="px-2.5 py-1 rounded bg-white border border-slate-200 shadow-xs text-slate-800">
                    q[{qubitIdx}] |0⟩
                  </span>
                </div>

                <div 
                  className="absolute left-24 top-1/2 h-[2px] bg-slate-300 z-0" 
                  style={{ width: `${stepCount * 64}px` }} 
                />
                  ...
                  {Array.from({ length: stepCount }, (_, stepIdx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`w-12 h-12 rounded-lg border border-dashed border-slate-300 bg-white/90 flex items-center justify-center cursor-pointer transition-all hover:border-blue-500 hover:bg-blue-50/50 shadow-xs ${
                        ...
                        {gateOnCell && (
                          <div className="relative group w-full h-full flex items-center justify-center">
                            {gateOnCell.qubit === qubitIdx && !['CNOT', 'CZ', 'TOFFOLI'].includes(gateOnCell.type) && (
                              <div className={`w-9 h-9 rounded ${getGateStyle(gateOnCell.type)} text-xs flex flex-col items-center justify-center font-bold shadow-xs`}>
                            ...
                            {gateOnCell.type === 'CNOT' && gateOnCell.qubit === qubitIdx && (
                              <div className="w-4 h-4 rounded-full bg-emerald-600 border border-white shadow-xs" />
                            )}
                            ...
                            {gateOnCell.type === 'TOFFOLI' && (gateOnCell.qubit === qubitIdx || gateOnCell.control2Qubit === qubitIdx) && (
                              <div className="w-4 h-4 rounded-full bg-amber-600 border border-white shadow-xs" />
                            )}
                            ...
                            <button
                              ...
                              className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-rose-600 border border-white text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20 shadow-xs cursor-pointer"
                              title="Delete gate"
                            >
                              ×
                            </button>
