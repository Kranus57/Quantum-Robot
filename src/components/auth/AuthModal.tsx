...
type AuthMethod = 'email' | 'google' | 'twilio';
  ...
  const handleSendTwilioOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setErrorMsg(res.message || 'Failed to send Twilio SMS OTP.');
      }
    } catch (err) {
      setErrorMsg('Twilio SMS service error.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderAuthFormContent = () => (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden w-full relative text-slate-900">
      ...
      <div className="p-6 space-y-4">
        {/* Auth Provider Selector (Email, Google, Twilio SMS) */}
        {authRoleTab === 'student' && (
          <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200">
            ...
            <button
              type="button"
              onClick={() => { setAuthMethod('twilio'); setErrorMsg(null); }}
              className={`py-2 rounded-lg text-[11px] font-bold transition-all flex items-center justify-center space-x-1.5 ${
                authMethod === 'twilio'
                  ? 'bg-white text-slate-900 border border-slate-200 shadow-sm'
                  : 'text-slate-600 hover:text-slate-900'
        ...
        {authMethod === 'google' && (
          <div className="space-y-4 p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
        ...
        {authMethod === 'twilio' && (
          <div className="space-y-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
            <div className="flex items-center space-x-2 text-slate-800 font-bold text-xs border-b border-slate-200 pb-2">
              ...
              <span>Twilio SMS Phone Authentication</span>
            </div>

            {otpStep === 'request' ? (
              <form onSubmit={handleSendTwilioOtp} className="space-y-3">
                <div>
                ...
                <div className="text-[11px] text-slate-600 bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm flex items-start space-x-2">
                  ...
                  <span>
                    A 6-digit verification code will be dispatched via Twilio SMS. Test Code: <strong className="text-blue-600 font-mono">123456</strong>
                  </span>
                </div>
              ...
              <form onSubmit={handleVerifyTwilioOtp} className="space-y-3 animate-fadeIn">
          ...
          <div className="lg:col-span-7 space-y-6 text-left pr-0 lg:pr-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
                Sign in with institutional credentials, <strong>Google SSO</strong>, or <strong>Twilio SMS OTP</strong> to access multi-framework quantum circuit simulation, 3D state visualization, and automated assessment tracking.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              ...
              <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-sm flex items-start space-x-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900">Google & Twilio Auth</h4>
                  <p className="text-[11px] text-slate-500 mt-0.5 font-medium">Secure Google SSO and Twilio SMS OTP verification.</p>
                </div>
              </div>
